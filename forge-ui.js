...
function recipeCardHtml(id,r){
 ...
 return `<article class="forge-recipe r-${r.rarity}" data-recipe="${id}"><h3>${r.parent?uiEscape(CRAFT_RECIPES[r.parent].name)+' +10 → ':''}${r.icon} ${uiEscape(r.name)}</h3><div>${r.rarity} / ${WEAPON_TYPES[r.weaponType]?.name||EQUIPMENT_SLOTS.find(s=>s.k===equipmentSlot(r))?.label||''} / ${uiEscape(BUILD_CATALOG[r.primaryBuildTag]||[...buildTags({item:r})].map(t=>BUILD_CATALOG[t]).filter(Boolean).slice(0,2).join('・'))}</div><p>${r.catalogKind} / ${forgeStatus(id,r)}</p><div class="forge-actions">${r.type==='weapon'?`<button class="btn btn-sub btn-xs" onclick="openWeaponTree('${id}')">派生ツリー</button>`:''}<button class="btn btn-gold btn-xs" onclick="openRecipeDetail('${id}')">詳細・${r.parent?'派生比較':'作成'}</button><button class="btn btn-sub btn-xs" aria-pressed="${!!p.favorites[id]}" onclick="toggleRecipeFavorite('${id}')">${p.favorites[id]?'★':'☆'} お気に入り</button></div></article>`;
}
...
function renderForgeUi(tab='create'){
 ...
 showChapterModal('🔨 鍛冶屋',`<nav class="forge-actions">${nav}</nav>${tab==='refine'?forgeRefineHtml():`<p style="font-size:12px">倉庫の未ロック素材を使用。親武器+10を消費。通常派生は+0、上位進化は強化値・追加特性を継承します。吸血は与ダメージ比例（率25%・最大HP8%上限）。ボスは半減し30Tで無効化。</p><details class="forge-filter-panel" ${window.innerWidth>=600&&window.innerHeight>500?'open':''}><summary>🔎 検索・絞り込み（部位 / レア度など）</summary>${filters}</details><div id="forge-results"></div>`}`,`${tab!=='refine'?'<div id="forge-paging" class="forge-actions"></div>':''}<button class="btn btn-sub btn-xs" onclick="closeGenericModal()">閉じる</button>`);
