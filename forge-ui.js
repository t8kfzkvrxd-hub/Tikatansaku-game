...
function renderForgeResults(){
 ...
 if(forgeTab==='create'&&forgeView==='recommended'){
  const rows=forgeRecommendations();
  target.innerHTML=rows.map(forgeRecommendationHtml).join('')||'<p>今すぐ作れる推薦候補はありません。「すべて」から目標を探すか、探索で素材を集めましょう。</p>';
