...
function recipeCardHtml(id,r){
 ...
 return `<article class="forge-recipe r-${r.rarity}" data-recipe="${id}"><h3>${r.parent?uiEscape(CRAFT_RECIPES[r.parent].name)+' +10 → ':''}${r.icon} ${uiEscape(r.name)}</h3><div>${r.rarity} / ${WEAPON_TYPES[r.weaponType]?.name||EQUIPMENT_SLOTS.find(s=>s.k===equipmentSlot(r))?.label||''} / ${uiEscape(BUILD_CATALOG[r.primaryBuildTag]||[...buildTags({item:r})].map(t=>BUILD_CATALOG[t]).filter(Boolean).slice(0,2).join('・'))}</div><p>${forgeStatus(id,r)}</p><div class="forge-actions"><button class="btn btn-gold btn-xs" onclick="openRecipeDetail('${id}')">詳細・${r.parent?'派生比較':'作成'}</button><button class="btn btn-sub btn-xs" aria-pressed="${!!p.favorites[id]}" onclick="toggleRecipeFavorite('${id}')">${p.favorites[id]?'★':'☆'} お気に入り</button></div></article>`;
}
