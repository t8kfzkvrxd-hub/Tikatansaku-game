...
function filteredForgeRecipes(){
 ...
 return FORGE_INDEX.filter(({id,r,category,search})=>{
  if(['awaken','tree'].includes(forgeTab)&&(r.type!=='weapon'||r.parent))return false;
  if(forgeTab==='create'&&forgeView==='armor'&&r.type!=='armor')return false;
...
function setForgeFilter(key,value){if(!(key in forgeFilters))return;forgeFilters[key]=value;forgeFilters.page=0;if(forgeView==='recommended'){forgeView='all';openCrafting(forgeTab);}else renderForgeResults();}
...
function recipeCardHtml(id,r){
 if(!forgeRecipeRevealed(id))return `<article class="forge-recipe" data-recipe="${id}"><h3>？？？</h3><p>🔒 特殊素材を発見すると解放</p></article>`;
...
function renderForgeUi(tab='create'){
 ...
 const characters=['player',...Object.keys(state.chapter.owned).filter(k=>state.chapter.owned[k])];
 const overview=tab==='create'?`<div class="character-tabs">${characters.map(id=>`<button class="btn btn-sub" aria-pressed="${forgeFilters.character===id}" onclick="forgeFilters.character='${id}';openCrafting('create')">${renderCharacterSD(id,{size:'small',selected:forgeFilters.character===id})}</button>`).join('')}</div>${forgeOverviewHtml()}<nav class="forge-actions" aria-label="レシピ分類">${FORGE_VIEWS.map(([id,label])=>`<button class="btn btn-sub" aria-pressed="${forgeView===id}" onclick="selectForgeView('${id}')">${label}</button>`).join('')}</nav>`:'';
 showChapterModal('🔨 鍛冶屋',`${overview}${tab==='refine'?forgeRefineHtml():`${tab==='create'&&forgeView==='recommended'?'':`<details class="forge-filter-panel" ${forgeView==='build'?'open':''}><summary>🔎 検索・絞り込み</summary>${filters}</details>`}<div id="forge-results"></div>`}<details class="forge-filter-panel"><summary>強化・派生・覚醒</summary><nav class="forge-actions">${nav}<button class="btn btn-sub" onclick="openForgeServices()">特性・高度な鍛冶</button></nav><p>倉庫の未ロック素材を使用。通常派生は親武器+10、覚醒は任意の強化値の親武器を消費。通常派生・覚醒は+0。覚醒すると強化値はリセットされます。継承対象の追加特性は維持します。</p></details>`,`${tab!=='refine'?'<div id="forge-paging" class="forge-actions"></div>':''}<button class="btn btn-sub btn-xs" onclick="openCharacterEquipment('${forgeFilters.character}')">装備を整える</button><button class="btn btn-sub btn-xs" onclick="closeGenericModal()">閉じる</button>`);
 document.querySelector('#modal-layer .update-notes-card')?.classList.add('forge-modal');
 if(tab==='create'&&forgeView==='recommended')document.querySelector('.forge-modal')?.classList.add('forge-recommended');
...
function forgeRefineHtml(){
 ...
 return `<p>強化は+10まで。キャラごとの装備を鍛錬できます。</p><div class="forge-actions">${chars.map(([k,l])=>`<button class="btn btn-${id===k?'gold':'sub'} btn-xs" onclick="forgeFilters.character='${k}';openCrafting('refine')">${renderCharacterSD(k,{size:'small',selected:id===k})}</button>`).join('')}</div>${facilityTier()<2?'<p>🔒 10Fクリア後に解禁</p>':EQUIPMENT_SLOTS.map(s=>{const i=slots[s.k];return i?`<article class="forge-recipe">${s.label}：${uiEscape(i.name)}<div>${uiEscape(getItemStatSummary(i))}</div><button class="btn btn-gold btn-xs" ${i.refineCount>=10||state.vaultGold<getRefineCost(i)?'disabled':''} onclick="refineCharacterItem('${id}','${s.k}')">${i.refineCount>=10?'MAX +10':`強化 ${getRefineCost(i)}G`}</button></article>`:'';}).join('')}`;
}
function refineCharacterItem(id,slot){if(state.screen!=='town'||facilityTier()<2)return;const item=characterEquipment(id)?.[slot];if(!item)return;refineEquipmentItem(item,slot);openCrafting('refine');}
function openRecipeDetail(id,parentId){
 const r=CRAFT_RECIPES[id];if(!forgeRecipeRevealed(id))return;
 ...
 if(!['accessory','accessory2'].includes(forgeFilters.accessory))forgeFilters.accessory='accessory';
 ...
 const parentControls=r.parent?`<p>必要強化：${r.awakening?'指定なし（覚醒後+0）':'+10'} / 親武器と素材を消費</p><select id="parent-${id}" aria-label="消費する親武器" onchange="openRecipeDetail('${id}',this.value)">${parents.map(i=>`<option value="${i.id}" ${i.id===parent?.id?'selected':''}>${uiEscape(i.name)} / ${equipmentOwner(i)==='player'?'主人公装備中':equipmentOwner(i)?'同行キャラ装備中':'倉庫'}</option>`).join('')}</select>`:'';
 showChapterModal(`${r.icon} ${uiEscape(r.name)}`,`<article class="forge-recipe forge-focused-detail"><section class="forge-decision"><p class="forge-play">${uiEscape((BUILD_GUIDES[r.primaryBuildTag]||'個別効果を確認してください。').split('。')[0])}</p>${forgeQuickCompare(forgeFilters.character,slot,r)}${buildChangeSummary(slots,slot,r)}${r.type==='weapon'?forgeNextPreview(id):''}</section><section class="forge-material-decision">${forgeMissingSources(r,id)}${Object.entries(r.materials).every(([k,n])=>materialCount(k)>=n)?'<p class="forge-ready">素材が揃っています</p>':''}<p>必要G ${r.gold} / 所持 ${state.vaultGold}</p>${issue?`<p>${uiEscape(issue)}</p>`:''}</section><section class="forge-secondary">${recipeFavoriteButton(id)}${parentControls}${r.awakening?`<p>覚醒すると強化値はリセットされます。<br>強化値：+${parent?.refineCount||0} → +0<br>覚醒後：${uiEscape(r.name)} +0</p>`:''}<details><summary>全能力比較・比較対象</summary>${compareSelect}${compareEquipmentHtml(current,r.inheritRefinement&&!r.awakening&&parent?{...r,baseAtk:(r.baseAtk||0)+(parent.refineCount||0)*3,affix:parent.affix}:r)}</details><details><summary>主・副ビルドと固有効果</summary>${weaponPlayHtml(r)}${buildItemDetails(r)}</details>${r.type==='weapon'?`<button class="btn btn-sub" onclick="openWeaponTree('${id}')">この先を見る</button>${weaponFuturesHtml(id)}<button class="btn btn-sub" onclick="setWeaponGoal('${id}')">この武器を目標にする</button>`:''}<details><summary>必要素材一覧</summary>${forgeMaterialHtml(r)}</details></section></article>`,`<button class="btn btn-gold btn-xs" ${issue?'disabled':''} onclick="craftEquipment('${id}',document.getElementById('parent-${id}')?.value)">${r.awakening?'覚醒':r.parent?'派生':'作成'}</button><button class="btn btn-sub btn-xs" onclick="openCrafting('${forgeTab}')">一覧へ戻る</button><button class="btn btn-sub btn-xs" onclick="closeGenericModal()">閉じる</button>`);
 document.querySelector('#modal-layer .update-notes-card')?.classList.add('forge-detail-modal');
