...
const FORGE_WEAPONS=[['all','すべて'],['sword','片手剣'],['greatsword','大剣'],['dagger','短剣'],['spear','槍'],['axe','斧'],['other','その他']];
...
const FORGE_INDEX=Object.entries(CRAFT_RECIPES).map(([id,r])=>({id,r,category:FORGE_WEAPONS.some(([k])=>k===r.archetype)?r.archetype:'other',search:[r.name,r.desc,...(r.tags||[]).flatMap(t=>[t,FORGE_TAG_NAMES[t]||'']),...Object.keys(r.materials).map(k=>MATERIALS[k]?.name||k)].join(' ').toLowerCase()}));
...
function recipeCardHtml(id,r){
 ...
 return `<article class="forge-recipe r-${r.rarity}" data-recipe="${id}"><h3>${r.parent?`${uiEscape(CRAFT_RECIPES[r.parent].name)} +10 → `:''}${r.icon} ${uiEscape(r.name)}</h3><div>${r.rarity} / ${label}　${forgeStatus(id,r)} ${!p.seen[id]?'🆕 新規解放':''}</div><div>${uiEscape(getItemStatSummary(r))}</div><div>${uiEscape(r.desc)} / ${uiEscape(effectDescription(r.effects))}</div>${forgeMaterialHtml(r)}<div class="${state.vaultGold<r.gold?'forge-missing':''}">必要G ${r.gold} / 所持 ${state.vaultGold}</div><div class="forge-actions"><button class="btn btn-gold btn-xs" onclick="openRecipeDetail('${id}')">${r.parent?'派生比較・詳細':'現在装備と比較・詳細'}</button>${!r.parent?`<button class="btn btn-sub btn-xs" ${issue?'disabled':''} onclick="craftEquipment('${id}')">${issue||'作成'}</button>`:''}<button class="btn btn-sub btn-xs" aria-pressed="${!!p.favorites[id]}" onclick="toggleRecipeFavorite('${id}')">${p.favorites[id]?'★':'☆'} お気に入り</button></div></article>`;
}
...
function renderForgeUi(tab='create'){
 ...
 showChapterModal('🔨 鍛冶屋',`<nav class="forge-actions">${nav}</nav>${tab==='refine'?forgeRefineHtml():`<p style="font-size:12px">倉庫の未ロック素材を使用。派生・覚醒は親武器+10を消費し、新装備は+0になります。</p><details class="forge-filter-panel" ${window.innerWidth>=600?'open':''}><summary>🔎 検索・絞り込み（部位 / レア度など）</summary>${filters}</details><div id="forge-results"></div>`}`,`${tab!=='refine'?'<div id="forge-paging" class="forge-actions"></div>':''}<button class="btn btn-sub btn-xs" onclick="closeGenericModal()">閉じる</button>`);
