...
function openHomeDataManagement(){
 ...
 showChapterModal('データ管理',`<p>セーブはこのブラウザに保存されます。クラウド保存・端末間同期・独立Webアプリとの自動同期はありません。</p><div class="card"><h3>iPhoneの起動方法</h3><p>既存セーブで遊ぶには、保存したSafariの通常ブラウザで開いてください。ホーム画面へ追加する際は「Webアプリとして開く」が表示される場合、オフにしてください。</p><p>別ブラウザ・別プロファイル・独立Webアプリでは、同じURLでも別の保存領域になることがあります。「つづきから」が出なくても、元の保存先のデータが消えたとは限りません。</p>${standalone?'<p class="home-warning">現在は独立Webアプリとして起動しています。Safari側のセーブとは自動同期されません。</p>':''}<p>確認のためにセーブ初期化・ブラウザデータ削除・既存Webアプリ削除を行わないでください。</p></div><p>初期化すると元に戻せません。</p><button class="btn btn-red" onclick="showResetSaveConfirmation()">セーブデータ初期化</button>`,`<button class="btn btn-sub" onclick="openHomeSettings()">設定へ戻る</button>`);
}
