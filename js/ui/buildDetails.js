const BUILD_AUDIT_RULES={
 ...
 break:['部位を狙って命中','部位破壊力×1.3。武器種・強攻撃倍率・装備固定値を別途適用。統合前の部位素材寄与8%→22%（全補正統合後に一度だけ＋2pt）、特定部位は敵ATK/DEF低下。強攻撃では敵DEF-3も適用','partBreakPower / hitEnemyPart / materialDropPlan'],
 ...
 farming:['敵撃破時','統合前の素材確率寄与+8pt。生存同行者と装備補正を合算、寄与50%上限（最終統合後＋2pt）。部位破壊＋生存参加者farmingで統合前のレア素材寄与+5pt','equipmentEffects / dropMonsterMaterials'],rareMaterial:['敵撃破時','統合前のレア素材確率寄与+8pt。生存同行者と装備値合算、寄与25%上限（最終統合後＋2pt）。未解禁レア度は出ない','equipmentEffects / dropMonsterMaterials'],chest:['素材報酬生成時（enemy以外）','品質抽選値-0.10。生存同行者と装備値合算。宝箱以外に採取等も対象。強制レア度は変更しない','createMaterialReward / partyChestQuality'],safe:['被弾時','戦闘ダメージ×0.95。探索イベントの直接HP減少は対象外','buildIncomingDamage'],deep:['61F以降の攻撃','ATK補正+20%','buildAttack'],abyssal:['500F以降の攻撃（現在未解放）','ATK補正+20%。現行200F内では発動しない・未完成','buildAttack']
};
