...
function battleBuildHtml(id){const v=buildView(id);if(!v)return '';return `<button class="build-mini" aria-label="${combatName(id)}のビルド詳細" onclick="event.stopPropagation();openBuildDetails('${id}')">詳細</button><div class="build-chips" hidden>${[...v.tags].sort((a,b)=>Number(b in v.conditions)-Number(a in v.conditions)).slice(0,3).map(t=>`<span class="build-state ${v.conditions[t]&&v.unit.hp>0?'active':''}">${v.conditions[t]&&v.unit.hp>0?'✦ ':''}${BUILD_CATALOG[t]}${t in v.conditions?(v.conditions[t]&&v.unit.hp>0?' 発動':' 待機'):' 条件時'}</span>`).join('')}</div>`;}
