const BattleUI={enemy:null,notices:[],builds:{},phase:null,partsOpen:false};
...
renderCombatReadout=function(){
 ...
 if(BattleUI.enemy!==enemy){BattleUI.enemy=enemy;BattleUI.partsOpen=false;BattleUI.notices=[];BattleUI.builds={};BattleUI.phase=enemy.phase;}
 ...
 enemyCard.insertAdjacentHTML('beforeend',`<div class="battle-intent"><small>次の行動</small><br>${uiEscape(enemy.pendingAction||'構え')}</div>`);
 ...
 root.insertAdjacentHTML('beforeend',`<div class="battle-enemy-art ${hit?'impact':''}" aria-label="${uiEscape(enemy.name)}">${picture?`<img src="${uiEscape(picture)}" alt="${uiEscape(enemy.name)}">`:`<span>${enemy.icon||'👹'}</span>`}<small>${uiEscape(enemy.traitName||enemy.hint||'')}</small></div><nav class="battle-toolbar"><button onclick="openBattleMenu()">道具・スキル</button><button onclick="openBattleLog()">ログ・発動</button>${expeditionPerkButton()}</nav><div class="battle-location">B${state.floor}F ${uiEscape(explorationArea()?.name||getCurrentArea().name||'')}</div>`);
 ...
 const parts=root.querySelector('.enemy-parts');
 if(parts){const disclosure=document.createElement('details');disclosure.className='battle-parts-disclosure';disclosure.open=BattleUI.partsOpen;const summary=document.createElement('summary');summary.textContent='攻撃対象：'+(PART_TYPES[enemy.partTarget]?.name||'本体');parts.before(disclosure);disclosure.append(summary,parts);disclosure.addEventListener('toggle',()=>{if(disclosure.isConnected)BattleUI.partsOpen=disclosure.open;});}
