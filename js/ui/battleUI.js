...
renderCombatReadout=function(){
 ...
 root.insertAdjacentHTML('beforeend',`<div class="battle-enemy-art ${hit?'impact':''}" aria-label="${uiEscape(enemy.name)}">${picture?`<img src="${uiEscape(picture)}" alt="${uiEscape(enemy.name)}">`:`<span>${enemy.icon||'👹'}</span>`}<small>${uiEscape(enemy.traitName||enemy.hint||'')}</small></div><nav class="battle-toolbar"><button onclick="openBattleMenu()">アイテム・特殊行動</button><button onclick="openBattleLog()">ログ・発動</button>${expeditionPerkButton()}</nav><div class="battle-location">B${state.floor}F ${uiEscape(explorationArea()?.name||getCurrentArea().name||'')}</div>`);
