...
function openEquipmentCandidate(itemId){
 ...
 showChapterModal('装備比較',`${renderCharacterSD(id,{selected:true,showLevel:true})}<h3>${uiEscape(item.name)}</h3>${compareCharacterTotals(id,slot,item)}${buildChangeSummary(characterEquipment(id),slot,item)}<details><summary>固有効果・装備単体の詳細</summary>${buildItemDetails(item)}${compareEquipmentHtml(current,item)}<p>現在のビルド：${uiEscape(gearBuildText(current))}</p><p>候補のビルド：${uiEscape(gearBuildText(item))}</p></details>${item.locked?'<p>🔒 ロック中（売却保護・装備は可能）</p>':''}`,`<button class="btn btn-sub" onclick="openCharacterEquipment('${id}','${slot}',${page})">戻る</button><button class="btn btn-gold" ${owner||state.screen!=='town'?'disabled':''} onclick="setCharacterEquipment('${id}','${slot}','${item.id}');openCharacterEquipment('${id}','${slot}',${page})">${state.screen!=='town'?'装備変更は拠点で':owner?uiEscape(name)+'が装備中':uiEscape(item.name)+'を装備'}</button>`);
}
function openCharacterEquipment(id='player',slot=null,page=0){
 ...
 const tabs=[['player','主人公'],...Object.keys(state.chapter.owned).filter(k=>state.chapter.owned[k]).map(k=>[k,CHARACTER_DATA[k]?.name||k])].map(([key,name])=>{const s=key==='player'?getPlayerStats():companionStats(key);return `<button class="btn character-tab ${key===id?'btn-gold':'btn-sub'}" aria-pressed="${key===id}" onclick="openCharacterEquipment('${key}')">${renderCharacterSD(key,{selected:key===id,showLevel:true})}</button>`;}).join('');
 const rows=EQUIPMENT_SLOTS.map(s=>`<div class="equipment-slot ${slot===s.k?'selected':''}"><button class="btn btn-sub" onclick="openCharacterEquipment('${id}','${s.k}')"><b>${s.icon} ${s.label}</b><span>${uiEscape(slots[s.k]?.name||'未装備')}</span></button>${slots[s.k]?`<button class="btn btn-sub" ${state.screen!=='town'?'disabled':''} onclick="removeCharacterEquipment('${id}','${s.k}');openCharacterEquipment('${id}','${s.k}')">解除</button>`:''}</div>`).join('');
 ...
 showChapterModal('👥 キャラ／装備',`<div class="gear-layout"><section class="gear-summary"><div class="character-tabs">${tabs}</div><p>HP ${stats.maxHp} / ATK ${stats.atk} / DEF ${stats.def}</p>${forgeOverviewHtml(id)}<details><summary>Lv・EXPの詳細</summary>${characterLevelHtml(id)}</details>${id!=='player'?`<details><summary>同行設定</summary><p>HP0では気絶。出撃時に回復。</p><button class="btn btn-sub" onclick="selectCompanion('${id}');openCharacterEquipment('${id}')">${state.chapter.companion===id?'同行中':'同行に選択'}</button><button class="btn btn-sub" onclick="selectCompanion(null);openCharacterEquipment('${id}')">同行を外す</button></details>`:''}</section><div class="equipment-slots">${rows}</div></div>${selection}`,`<button class="btn btn-sub" onclick="openCharacterStatus('${id}')">ステータス</button><button class="btn btn-sub" onclick="closeGenericModal()">閉じる</button>`);
 document.querySelector('#modal-layer .update-notes-card')?.classList.add('equipment-modal');
};
