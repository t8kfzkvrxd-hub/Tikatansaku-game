...
function openEquipmentCandidate(itemId){
 ...
 showChapterModal('装備比較',`${renderCharacterSD(id,{selected:true,showLevel:true})}${buildItemDetails(item)}${compareCharacterTotals(id,slot,item)}${compareEquipmentHtml(current,item)}<p>現在のビルド：${uiEscape(gearBuildText(current))}</p><p>候補のビルド：${uiEscape(gearBuildText(item))}</p><p>下段は装備単体の差分です。条件付き効果は発動条件も確認してください。</p>${item.locked?'<p>🔒 ロック中（売却保護・装備は可能）</p>':''}`,`<button class="btn btn-sub" onclick="openCharacterEquipment('${id}','${slot}',${page})">戻る</button><button class="btn btn-gold" ${owner||state.screen!=='town'?'disabled':''} onclick="setCharacterEquipment('${id}','${slot}','${item.id}');openCharacterEquipment('${id}','${slot}',${page})">${state.screen!=='town'?'装備変更は拠点で':owner?uiEscape(name)+'が装備中':uiEscape(item.name)+'を装備'}</button>`);
}
function openCharacterEquipment(id='player',slot=null,page=0){
 ...
 const tabs=[['player','主人公'],...Object.keys(state.chapter.owned).filter(k=>state.chapter.owned[k]).map(k=>[k,CHARACTER_DATA[k]?.name||k])].map(([key,name])=>{const s=key==='player'?getPlayerStats():companionStats(key);return `<button class="btn character-tab ${key===id?'btn-gold':'btn-sub'}" aria-pressed="${key===id}" onclick="openCharacterEquipment('${key}')">${renderCharacterSD(key,{selected:key===id,showLevel:true})}<small>HP ${s.maxHp} / ATK ${s.atk} / DEF ${s.def}</small></button>`;}).join('');
 ...
 showChapterModal('👥 キャラ／装備',`<div class="character-tabs">${tabs}</div>${completedBuildSummary(id)}${characterLevelHtml(id)}<p>総能力 HP ${stats.maxHp} / ATK ${stats.atk} / DEF ${stats.def}</p><div class="equipment-slots">${rows}</div>${selection}${id!=='player'?`<p>独立HPで通常戦闘に参加。HP0では気絶、出撃時に回復します。</p><button class="btn btn-sub" onclick="selectCompanion('${id}');openCharacterEquipment('${id}')">${state.chapter.companion===id?'同行中':'同行に選択'}</button><button class="btn btn-sub" onclick="selectCompanion(null);openCharacterEquipment('${id}')">同行を外す</button>`:''}`,`<button class="btn btn-sub" onclick="openCharacterStatus('${id}')">ステータス</button><button class="btn btn-sub" onclick="closeGenericModal()">閉じる</button>`);
};
