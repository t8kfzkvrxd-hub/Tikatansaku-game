...
function createMaterialReward(source='chest',floor=state.floor,forcedRarity=null,rng=Math.random){
 ...
 const roll=Math.max(0,rng()-(source!=='enemy'?partyChestQuality()/100:0)-(floor>100?Math.min(.05,(floor-100)*.0005):0)),tier=Math.min(materialTierLimit(),forcedRarity?Math.max(0,MATERIAL_MILESTONES.findIndex(r=>r.rarity===forcedRarity)):source==='cursed_chest'?Math.max(2,roll<.2?materialTierLimit():2):roll<.05?materialTierLimit():roll<.25?2:roll<.75?1:0);
